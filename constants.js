// ============================================================
// CONSTANTS.JS - All the definitions for the game
// ============================================================
// This file is like the "dictionary" of the game.
// It defines every block, item, mob, and recipe.
// Nothing here DOES anything - it just describes things.
// ============================================================

// --- CANVAS (our drawing board) ---
const canvas = document.getElementById("gameCanvas");
const ctx = canvas.getContext("2d");

// --- BLOCK IDs (0-49) ---
// Blocks are things that exist in the world grid.
const BLOCKS = {
    AIR: 0, GRASS: 1, DIRT: 2, STONE: 3, COAL: 4, IRON: 5,
    GOLD: 6, DIAMOND: 7, WOOD: 8, LEAVES: 9, BEDROCK: 10,
    SAND: 11, WATER: 12, PLANKS: 13, COBBLESTONE: 14,
    BED: 15,
    // Biome blocks
    SNOW: 16, ICE: 17, CACTUS: 18, DRY_GRASS: 19,
    SPRUCE_WOOD: 20, SPRUCE_LEAVES: 21,
    ACACIA_WOOD: 22, ACACIA_LEAVES: 23,
    TORCH: 24,
    EMERALD: 25,
    CHEST: 26,
    // Doors
...
// --- ITEM IDs (100+) ---
// Items are things you hold but can't place in the world.
const ITEMS = {
    STICK: 100,
    WOODEN_PICKAXE: 101, STONE_PICKAXE: 102, IRON_PICKAXE: 103,
    GOLD_PICKAXE: 104, DIAMOND_PICKAXE: 105,
    WOODEN_SWORD: 106, STONE_SWORD: 107, IRON_SWORD: 108,
    GOLD_SWORD: 109, DIAMOND_SWORD: 110,
    WOODEN_AXE: 111, STONE_AXE: 112, IRON_AXE: 113,
    GOLD_AXE: 114, DIAMOND_AXE: 115,
    RAW_PORKCHOP: 116, BONE: 117, GUNPOWDER: 118, ROTTEN_FLESH: 119,
    // Armor (120-131)
    IRON_HELMET: 120, IRON_CHESTPLATE: 121, IRON_LEGGINGS: 122, IRON_BOOTS: 123,
    GOLD_HELMET: 124, GOLD_CHESTPLATE: 125, GOLD_LEGGINGS: 126, GOLD_BOOTS: 127,
    DIAMOND_HELMET: 128, DIAMOND_CHESTPLATE: 129, DIAMOND_LEGGINGS: 130, DIAMOND_BOOTS: 131,
    // Mob drops
...
const SURFACE_LEVEL = 35;
const SEA_LEVEL = 42;
const GRAVITY = 0.45;
const MAX_FALL_SPEED = 12;
const PLAYER_REACH = 5.5; // How far the player can reach (in blocks)
const SAFE_FALL_BLOCKS = 3; // Fall this many blocks without damage
const NIGHT_THRESHOLD = 0.4; // dayBrightness below this = nighttime
const TORCH_LIGHT_RADIUS = 5; // blocks of light around a torch
const TORCH_SPAWN_RADIUS = 8; // blocks where mobs can't spawn near a torch

// --- UI LAYOUT (shared between rendering and input) ---
const UI = {
    CRAFTING_PANEL_W: 750,
    CRAFTING_PANEL_H: 600,
    RECIPE_W: 320,
    RECIPE_H: 52,
    RECIPE_COLS: 2,
    RECIPE_GAP: 15,
    RECIPE_START_X: 20,  // offset from panel left
    RECIPE_START_Y: 70,  // offset from panel top
    SLOT_SIZE: 36,
    SLOT_PAD: 3,
    INV_COLS: 9,
    INV_OFFSET_X: -30,   // shift inventory left to make room for armor
    ARMOR_GAP: 20,       // gap between inventory and armor slots
    INV_BOTTOM_MARGIN: 120, // space at bottom for inventory area
    HOTBAR_ROW_Y: -90,   // offset from panel bottom
    BACKPACK_ROW_Y: -50,  // offset from panel bottom
    // Trading menu
    TRADING_PANEL_W: 450,
    TRADING_PANEL_H: 500,
    TRADE_ROW_H: 46,
    ...
    CHEST_SLOTS: 9,
    CHEST_SLOT_COLS: 9,
    CHEST_SLOT_START_Y: 70,
};

// Computed UI values (so they aren't recalculated everywhere)
UI.INV_TOTAL_W = UI.INV_COLS * (UI.SLOT_SIZE + UI.SLOT_PAD) - UI.SLOT_PAD;

// --- GAME STATE MACHINE ---
// "menu" = title screen, "playing" = in game, "paused" = pause overlay
// "generating" / "loading" / "saving" = transient states
let gameState = "menu";

// --- SAVE SYSTEM ---
const SAVE_KEY_PREFIX = "lm2d_save_";
const SAVE_INDEX_KEY = "lm2d_saveIndex";
const MAX_SAVES = 10;

// Menu button hit areas (set at draw time, read by input.js)
const MENU_BUTTONS = { newWorld: null, savedWorlds: [] };
const PAUSE_BUTTONS = { resume: null, saveQuit: null };
let menuHover = null;
let menuSaveList = [];
let menuScrollOffset = 0;

// --- BIOME DEFINITIONS ---
const BIOMES = { FOREST: 0, DESERT: 1, SAVANNAH: 2, TUNDRA: 3 };
...
function isTool(id) { return id >= 101 && id <= 115; }
function isArmor(id) { return id >= 120 && id <= 131; }
function isFood(id) { return id === ITEMS.RAW_PORKCHOP || id === ITEMS.ROTTEN_FLESH || id === ITEMS.MUTTON || id === ITEMS.STEAK || id === ITEMS.RAW_CHICKEN; }
...
// --- BLOCK PROPERTIES ---
// toolType = which tool mines it fastest
// minTier = minimum tool tier needed (0=hand, 1=wood, 2=stone, 3=iron, 4=diamond)
// drops = what you get when you mine it
...
// --- MOB DEFINITIONS ---
// Each mob type: what it looks like, how it behaves, what it drops
const MOB_DEFS = {
    zombie: {
        name: "Zombie", width: 24, height: 46,
        maxHealth: 20, speed: 0.5, damage: 3,
        hostile: true, detectRange: 16, attackRange: 32,
        knockback: 5,
        drops: [{ id: ITEMS.ROTTEN_FLESH, min: 0, max: 2 }]
    },
    skeleton: {
        name: "Skeleton", width: 22, height: 46,
        maxHealth: 20, speed: 1.3, damage: 3,
        hostile: true, detectRange: 16, attackRange: 450,
        shootInterval: 2000, knockback: 4,
        drops: [{ id: ITEMS.BONE, min: 0, max: 2 }]
    },
    creeper: {
        name: "Creeper", width: 20, height: 42,
        maxHealth: 20, speed: 1.0, damage: 12,
        hostile: true, detectRange: 16,
        fuseRange: 96, fuseTime: 1500, explosionRadius: 3,
        knockback: 8,
        drops: [{ id: ITEMS.GUNPOWDER, min: 0, max: 2 }]
    },
    pig: {
        name: "Pig", width: 30, height: 22,
        maxHealth: 10, speed: 0.8, damage: 0,
        hostile: false, knockback: 3,
        drops: [{ id: ITEMS.RAW_PORKCHOP, min: 1, max: 3 }]
    },
    cow: {
        name: "Cow", width: 30, height: 22,
        maxHealth: 10, speed: 0.8, damage: 0,
        hostile: false, knockback: 3,
        drops: [{ id: ITEMS.LEATHER, min: 1, max: 2 }, { id: ITEMS.STEAK, min: 1, max: 3 }]
    },
    sheep: {
        name: "Sheep", width: 28, height: 20,
        maxHealth: 8, speed: 0.9, damage: 0,
        hostile: false, knockback: 3,
        drops: [{ id: ITEMS.WOOL, min: 1, max: 3 }, { id: ITEMS.MUTTON, min: 1, max: 2 }]
    },
    villager: {
        name: "Villager", width: 24, height: 46,
        maxHealth: 20, speed: 0.4, damage: 0,
        hostile: false, knockback: 2,
        drops: []
    },
    husk: {
        name: "Husk", width: 24, height: 46,
        maxHealth: 22, speed: 0.8, damage: 4,
        hostile: true, detectRange: 16, attackRange: 32,
        knockback: 5,
        drops: [{ id: ITEMS.ROTTEN_FLESH, min: 0, max: 2 }]
    },
    enderman: {
        name: "Enderman", width: 20, height: 56,
        maxHealth: 40, speed: 1.5, damage: 7,
        hostile: true, detectRange: 12, attackRange: 40,
        knockback: 6, teleportCooldown: 3000,
        drops: [{ id: ITEMS.ENDER_PEARL, min: 0, max: 1 }]
    },
    spider: {
        name: "Spider", width: 32, height: 18,
        maxHealth: 16, speed: 1.2, damage: 3,
        hostile: true, detectRange: 12, attackRange: 32,
        knockback: 4,
        drops: [{ id: ITEMS.FEATHER, min: 0, max: 2 }]
    },
    chicken: {
        name: "Chicken", width: 16, height: 16,
        maxHealth: 4, speed: 0.7, damage: 0,
        hostile: false, knockback: 5,
        drops: [{ id: ITEMS.FEATHER, min: 1, max: 2 }, { id: ITEMS.RAW_CHICKEN, min: 1, max: 1 }]
    }
};
...
const RECIPES = [
    ...
    // Flint & Steel - light nether portals!
...
// --- VILLAGER TRADES ---
// Each trade: pay emeralds, receive items
const TRADES = [
